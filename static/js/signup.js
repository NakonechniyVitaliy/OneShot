$(document).ready(() => {
   ...
   $('#login').blur(() => {
        ...
        if (regExp1.test(loginX)) {
            ...
            correct1 = false;
            console.log('Логин не відповідає вимогам безпеки!');
   ...
   $('#email').blur(() => {
        ...
        if (regExp3.test(emailX)) {
            $.ajax({
                ...
                success: function(response) {
                    if (response.email_message == 'зайнятий'){
                        console.log('Даний email зайнятий! Спробуйте інший');
            ...
            correct3 = false;
            console.log('Не коректний формат Email!');
